...
<?php
    ...
    $result=mysql_query("select id from department_id");
	if($result == false){
		echo mysql_error();
		die();
	}
	...
	while($resultingArray = mysql_fetch_array($result)){
		$UniversityDepartmentID[] = $resultingArray[0];
	}
	...
	$kodikos_sxolhs = trim($allDataInSheet[$i]["L"]);
		...
		if(!in_array($kodikos_sxolhs,$UniversityDepartmentID)){
			continue ;
		}
	...
	//
	$insertTable = "insert into userinfo (department_ID, sc_ardeltiou ,last,first,fname,mname,birthdate,sex,sc_schapof,in_modeID,sc_arseiras,in_points,haddress,hzip,hphone,mobilephone,email,`school_name`, `school_type`, `orientation`, `pref`, `gvp`, `amav`,`sc_name`,`sc_code`)  values('";
	...
	$insertTable .= $email ."','";
	$insertTable .= $onomasia_sxoleiou ."','";
	$insertTable .= $leitourgia_sxoleiou ."','";
	$insertTable .= $kateuthinsi_code ."','";
	$insertTable .= $seira_pritimhshs ."','";
	$insertTable .= $gvp ."','";
	$insertTable .= $amav ."','";
	$insertTable .= $onomasia_sxolhs ."','";
	$insertTable .=  $kodikos_sxolhs."');";
